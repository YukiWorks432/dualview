...
const mainModes: ModeConfig[] = [
  ...
  },
  {
    ...
    label: 'Prompt',
    shortcut: '5',
    description: 'Compare text/prompts',
  },
  {
    mode: 'json-diff',
    icon: Braces,
  ...
  },
  {
    ...
    label: '3D',
    shortcut: '7',
    description: 'Compare GLB/GLTF models',
  },
  {
    mode: 'document',
    ...
    label: 'Document',
    shortcut: '8',
    description: 'Compare CSV, Excel, DOCX, PDF',
  },
]
...
const moreModes: { name: string; description: string; modes: ModeConfig[] }[] = [
  {
    ...
    modes: [
      { mode: 'split', icon: Layers, label: 'Split Screen', description: '2x2 grid layout' },
      {
        ...
        label: 'Quad View',
        shortcut: 'Q',
        description: '4-way comparison (MODE-001)',
      },
    ],
  },
  {
    ...
    modes: [
      ...
      { mode: 'heatmap', icon: Flame, label: 'Heatmap', description: 'See pixel differences' },
      {
        ...
        label: 'Radial Loupe',
        shortcut: 'R',
        description: 'Magnifier comparison (MODE-002)',
      },
      {
        mode: 'grid-tile',
        icon: Grid,
        label: 'Grid Tile',
        shortcut: 'G',
        description: 'Checkerboard A/B (MODE-003)',
      },
      {
        mode: 'morphological',
        icon: Shrink,
        label: 'Morphological',
        shortcut: 'M',
        description: 'Apply morph operations (MODE-005)',
      },
    ],
  },
]
    ...
    <header className="h-12 bg-surface border-b border-border flex items-center justify-between px-2 md:px-3">
      ...
      >
        ...
        <div className="relative group" ref={menuRef}>
          ...
          >
            ...
            <ChevronDown
              className={`w-2.5 h-2.5 transition-transform duration-200 ${showMoreMenu ? 'rotate-180' : ''}`}
            />
          </Button>
                ...
                <div key={category.name}>
                  ...
                  {category.modes.map(({ mode, icon: Icon, label, description }) => (
                    ...
                    >
                      ...
                      {comparisonMode === mode && <span className="w-2 h-2 bg-accent" />}
                    </button>
                  ))}
                </div>
              ))}
      ...
      {showMobileMenu && (
        <>
          ...
          />
          {/* Menu */}
          <div ref={mobileMenuRef} className="mobile-mode-menu safe-area-bottom">
